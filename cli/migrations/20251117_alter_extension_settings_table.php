...
try {
    ...
    $extensionColumns = tableColumns($connection, 'extensions');
    if (!in_array('installed_version', $extensionColumns, true)) {
        $connection->exec("ALTER TABLE extensions ADD COLUMN installed_version VARCHAR(20) NULL AFTER version");
    }
    $connection->commit();
    echo "Extension metadata tables migrated to latest structure." . PHP_EOL;
